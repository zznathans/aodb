"""Legacy AOML contract - raw chat-markup text, for game-chat clients that
expect that shape rather than JSON. See api.py for the primary JSON API."""
...
async def search(
    ...
    # Legacy clients never check the HTTP status code and show the raw body
    # verbatim in chat - so even error cases must return 200 with a body,
    # never a 4xx/5xx that would otherwise be dropped silently by the client.
    if output != "aoml":
